...
def cleanup_expired() -> int:
    ...
    with _cache_lock:
        now = time.time()
        keys_to_remove = [k for k, (_, expiry) in _cache.items() if expiry <= now]
        for key in keys_to_remove:
            del _cache[key]
        return len(keys_to_remove)
...
def cache_stats() -> dict[str, Any]:
    ...
    with _cache_lock:
        now = time.time()
        total = len(_cache)
        expired = sum(1 for _, (_, expiry) in _cache.items() if expiry <= now)
        return {
            'total_entries': total,
...
def cached(prefix: str, ttl: int = DEFAULT_TTL) -> Callable:
    ...
    Usage:
        @cached('filtered_readings', ttl=60)
        def get_filtered_readings(filters):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            # Skip 'self' or 'info' arguments for key generation
            cache_args = args
            if args and hasattr(args[0], '__class__'):
                # Skip self/cls for methods
                if args[0].__class__.__name__ in ('Query', 'Mutation'):
                    cache_args = args[1:]  # Skip self
                    if cache_args and hasattr(cache_args[0], 'context'):
                        cache_args = cache_args[1:]  # Skip GraphQL info
